...
Play / Mute / Stop removed from the home and bottom rows — they were already on
the right thumb cluster. Bri+ removed (use OS shortcut). Duplication eliminated.

**Changed: TD(4) redesigned.**
Was tap=`~/` · double=`` ` `` (L3 inner column).
Now tap=`` ` `` (backtick dead-key fix) · double=` ``` ` (Markdown code fence).
Serves both L3 inner column and L10 middle finger. `~/` is fully covered by
TD(3) on L9.
